...
def generate_polygon(
    ...
    for i in range(num_sides):
        ...
        this_coord = np.array(
            (radius * math.sin(i * angle), radius * math.cos(i * angle))
        )
...
class TestRingFinder:
    ...
    def test_two_squares(self):
        ...
        """
        G = nx.Graph()
        G.add_edges_from([[0, 1], [1, 2], [2, 3], [3, 0], [2, 5], [4, 5], [3, 4]])
        coords_dict = {
            0: np.array([0, 0]),
            1: np.array([0, 1.0]),
            2: np.array([1.0, 1.0]),
...
class TestPeriodicRingFinder:
    ...
    def test_three_squares(self):
        ...
        G = nx.Graph()
        G.add_edges_from(
            [[0, 1], [1, 2], [2, 3], [3, 0], [2, 5], [4, 5], [3, 4], [5, 1], [4, 0]]
        )
        coords_dict = {
            0: np.array([0, 0]),
            1: np.array([0, 1.0]),
        ...
        rf = PeriodicRingFinder(
            graph=G, coords_dict=coords_dict, cell=np.array([2.5, 2.5])
        )
    ...
    def test_2d_squares(self):
        ...
        rf = PeriodicRingFinder(
            graph=G, coords_dict=coords_dict, cell=np.array([3.0, 3.0])
        )
...
class TestRealData:
    ...
    def test_procrystalline(self):
        ...
        ring_finder = PeriodicRingFinder(G, COORDS_DICT, np.array([10.0, 10.0]))
    ...
    def test_collagen(self):
        ...
        correct_counter = Counter(rings)
        ring_finder = PeriodicRingFinder(G, COORDS_DICT, np.array([100.0, 100.0]))
        this_counter = Counter([len(ring) for ring in ring_finder.current_rings])
        for item in this_counter.keys():
            assert correct_counter[item] == this_counter[item]
    ...
    def test_two_perimeter(self):
        ...
        correct_counter = {4: 7, 6: 5, 8: 3, 10: 2, 14: 2, 18: 1, 22: 1, 42: 1}
        ring_finder = PeriodicRingFinder(
            G, COORDS_DICT, np.array([81.00977790682138, 81.00977790682138])
        )
        this_counter = Counter([len(ring) for ring in ring_finder.current_rings])
        for item in this_counter.keys():
            assert correct_counter[item] == this_counter[item]
    ...
    def test_doublecount_large(self):
        ...
        """
        G = nx.read_edgelist(
            "./data/doublecount_large_edges.dat",
            comments="#",
            delimiter=",",
            nodetype=int,
        )
        ...
        with open("./data/doublecount_large_coords.dat", "r") as fi:
            for i, line in enumerate(fi.readlines()):
                ...
                COORDS_DICT[node_id] = np.array([x, y])
        ring_finder = PeriodicRingFinder(
            G, COORDS_DICT, np.array([83.00977790682138, 83.00977790682138])
        )
        this_counter = Counter([len(ring) for ring in ring_finder.current_rings])
